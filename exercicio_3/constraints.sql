--Usúario
	...
	--ADD CONSTRAINTS
		...
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_cpf CHECK(exercicio3.is_valid_cpf(cpf));
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_cnpj CHECK(exercicio3.is_valid_cnpj_alpha(cnpj));
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_email CHECK(
			email ~ '^[^.]{1}(?!.*\.\.)[A-Za-z0-9._%+-]{0,62}[^.]{1}@(?!.*\.\.)[^._%+-]{1}[A-Za-z0-9.-]{0,252}\..*[^._%+-]{1}$'
		);
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_password CHECK(
			password ~ '^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*\W)[^ ]{8,}.*$'
			AND 
			password != email
		);
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_phonenumber CHECK(
			(
		...
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_age CHECK(age BETWEEN 14 AND 120);
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_user_status CHECK(
			user_status IN ('ativo', 'inativo', 'bloqueado', 'pendente')
		);
		ALTER TABLE exercicio3.t_user ADD CONSTRAINT exercicio3_t_user_ck_acess_tier CHECK(access_tier BETWEEN 1 AND 5);
...
--Produtos
	...
	ALTER TABLE exercicio3.t_products ADD CONSTRAINT exercicio3_t_products_ck_stock CHECK(
